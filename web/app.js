...
async function init(){
 ...
 document.title='Kim Hsu｜.NET 開發者與系統架構實作者';for(const [id,key]of Object.entries({'collection':'collection','subtitle':'subtitle','description':'description','tagline':'tagline','technique':'technique','edition':'edition'}))if(config[key]&&$(id))$(id).textContent=config[key];
...
function setupControls(){
 ...
 const openAbout=()=>$('about').showModal();const details=$('details');if(details)details.onclick=openAbout;$('soundless').onclick=openAbout;$('about').querySelector('.close').onclick=()=>$('about').close();
}
